...
"true");var d=b.find("div.desc-field");0<d.length?(b.find("div.description").removeClass("closed"),d.attr("contentEditable","true"),0==$("div.wysiwyg").length&&(b=new Wysiwyg,d.before(b.el))):(b.append('<div class="description"><div class="desc-field" contentEditable="true"></div></div>'),0==$("div.wysiwyg").length&&(b=new Wysiwyg,$("div.desc-field").before(b.el)))});$("a.delete-event").live("click",function(){var b=$(this).parents(":eq(4)"),d=b.find("button.edit-event").attr("id");void 0!=d?$.ajax({type:"GET",
url:config.remove,dataType:"json",data:{id:d},success:function(c){"deleted"==c.result?(b.parent().response(ui_lang.cal_evt_deleted,!0),change_count("-"),b.remove()):b.response(ui_lang.cal_error_del,!1)},fail:function(){alert(ui_lang.fail)}}):alert(ui_lang.finish_editing)});$("a.done-event").live("click",function(){var b=$(this).parents(":eq(4)"),d=b.find("button.edit-event").attr("id");void 0!=d?$.ajax({type:"GET",dataType:"json",url:config.done,data:{id:d},success:function(c){"done"==c.result?(b.find("h3.event-name").addClass("event-done"),
b.find("a.done-event").addClass("hide"),b.find("a.not-done-event").removeClass("hide")):b.response(c.result,!1)},fail:function(){alert(ui_lang.fail)}}):alert(ui_lang.finish_editing)});$("a.not-done-event").live("click",function(){var b=$(this).parents(":eq(4)"),d=b.find("button.edit-event").attr("id");void 0!=d?$.ajax({type:"GET",url:config.not_done,dataType:"json",data:{id:d},success:function(c){"done"==c.result?(b.find("h3.event-name").removeClass("event-done"),b.find("a.done-event").removeClass("hide"),
b.find("a.not-done-event").addClass("hide")):b.response(c.result,!1)},fail:function(){alert(ui_lang.fail)}}):alert(ui_lang.finish_editing)});$("button.save-event").live("click",function(){var b=$(this).parents(":eq(2)");time_el=b.find("span.time");event_el=b.find("h3.event-name");desc_el=b.find("div.desc-field");var d=$(this).attr("id"),c=$("h2#active-date").html().slice(0,-1),e=time_el.html(),f=event_el.html(),h=desc_el.html(),g=$(this);""!=c&""!=f?$.ajax({type:"POST",dataType:"json",url:config.edit,
data:{id:d,day:c,time:e,data:f,description:h},success:function(c){switch(c.result){case "error":b.response(ui_lang.cal_couldnt_save,!1);time_el.attr("contentEditable","false");event_el.attr("contentEditable","false");desc_el.attr("contentEditable","false");g.removeClass("save-event btn-success");g.addClass("btn-primary edit-event");g.html(ui_lang.edit);break;case "reserved":b.response(ui_lang.cal_time_reserved);break;default:time_el.attr("contentEditable","false"),event_el.attr("contentEditable",
"false"),desc_el.attr("contentEditable","false"),$("div.wysiwyg").remove(),g.removeClass("save-event btn-success"),g.addClass("btn-primary edit-event"),g.html(ui_lang.edit),$("div#event-container > div").tsort("span.time")}},fail:function(){b.response(ui_lang.fail,!1);time_el.attr("contentEditable","false");event_el.attr("contentEditable","false");desc_el.attr("contentEditable","false")}}):alert(ui_lang.no_values);desc_button()});$(document).on("hover",".change-event",function(){$(this).datepicker("show");
$(document).on("hover",".delete-event",function(){$(".change-event").datepicker("hide")});$(document).on("hover",".done-event",function(){$(".change-event").datepicker("hide")});$(this).datepicker().on("changeDate",function(b){var d=$(this).parents(":eq(4)"),c=d.find("button.edit-event").attr("id"),e=getFormatedDate(b.date);if(void 0!=c){if(b.date.getTime()==getSelDate().getTime())return!1;$.ajax({type:"GET",dataType:"json",url:config.changedate,data:{id:c,newdate:e},success:function(c){"changed"==
c.result?(d.parent().response(ui_lang.cal_date_changed+e,!0),change_count("-"),b.date.getMonth()==getSelDate().getMonth()&&find_increment(b.date.getDate()),d.remove()):d.response(ui_lang.cal_error_change,!1)},fail:function(){alert(ui_lang.fail)}})}else alert(ui_lang.finish_editing);$(this).datepicker("hide");b.stopPropogation()})});$(".show-small-cal").on("click",function(){$(this).datepicker({viewMode:1}).on("changeMonth",function(b){$(this).datepicker("hide");location.href=config.home+"simplecal/show/"+
b.date.getFullYear()+"/"+getZBM(b.date)});$(this).datepicker("show")});$("#show-add").live("click",function(){$("#event-container").hide();$(".calendar .cal-cell").popover("hide");$("#add-section").fadeIn("fast");if(0==$("div.wysiwyg").length){var b=new Wysiwyg;$(".area").before(b.el)}$(this).hide();ready_time($("input#time"))});$("#hide-add").live("click",function(b){b.preventDefault();$("div.wysiwyg").remove();$("#add-section").hide();$("#show-add").fadeIn("fast");$("#event-container").fadeIn("fast")});
$("#main-add").live("click",function(b){b.preventDefault();b=$("h2#active-date").html().slice(0,-1);var d=$("#time").val(),c=$("#event").val(),e=$("div.area")[0].innerHTML;""!=b&""!=c?$.ajax({type:"POST",url:config.add_event,dataType:"json",data:{day:b,time:d,event:c,description:e},success:function(b){"Added"==b.result?(change_count("+"),add_event(b.id,d,c,e),$("p.event-info").remove(),$("div#event-container > div").tsort("span.time"),$("#hide-add").trigger("click"),$("div#event-container").response(ui_lang.cal_added,
!0)):$(".popover-add").response(b.result,!1)},fail:function(){$(".popover-add").response(ui_lang.server_side_error,!1)}}):$(".popover-add").response(ui_lang.no_values,!1)});$("form :input").change(function(){$(this).closest("form").data("changed",!0)});$("ul.nav-tabs li a").click(function(){$.totalStorage("opened_tab",$(this).attr("href"))});$("#userSettings").click(function(b){b.preventDefault();b=$("#user_settings");if($(this).closest("form").data("changed")){var d=!1;$.each(b.find("input"),function(){if(""==
this.value||void 0==this.value)d=!0});d?$("#user_settings").response(ui_lang.no_values,!1):(b=$("#user_settings"),b.attr("action",config.home+"settings/process_user"),b.submit())}else $("#user_settings").response(ui_lang.no_change,!1)});$("a.bt_hide").live("click",function(b){b.preventDefault();b=$(this).attr("href");$(b).toggle();$(b).next().toggle();$(this).children("i").toggleClass("icon-chevron-up");$(this).children("i").toggleClass("icon-chevron-down");set_hidden(b)});$("#delete_acc_button").click(function(){if(confirm(ui_lang.delete_acc))window.location=
config.home+"logout/app/delete";else return!1});$("#access_log").click(function(){$.get(config.access,function(b){b=eval(b);b[0].message||(2>$("#accessTable tr").length&&b.forEach(function(b){$("#accessTable").append("<tr><td>"+b.ip+"</td><td>"+b.time+"</td><td>"+b.client+"</td></tr>")}),$("#accessModal").modal("show"))})})});
jQuery.fn.forcetime=function(){return this.each(function(){$(this).focusout(function(){""==this.innerHTML&&(this.innerHTML="0:00")});$(this).keydown(function(b){b=b.charCode||b.keyCode||0;return 8==b||9==b||46==b||186==b||37<=b&&40>=b||48<=b&&57>=b||96<=b&&105>=b})})};function hide_hidden(){var b=$.totalStorage("hidden_sections");if($.isArray(b))for(var d in b)$(b[d]).hide(),$(b[d]).next().hide(),$("a[href$="+b[d]+"]").children("i").attr("class","icon-chevron-down")}
function set_hidden(b){var d=[],d=$.totalStorage("hidden_sections");$.isArray(d)?-1==$.inArray(b,d)?d.push(b):(b=d.indexOf(b),d.splice(b,1)):(d=[],d.push(b));$.totalStorage("hidden_sections",d)}

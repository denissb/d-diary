...
!1)},fail:function(){alert(ui_lang.fail)}})}else alert(ui_lang.finish_editing);$(this).datepicker("hide");b.stopPropogation()})});$("#show-add").live("click",function(){$("#event-container").hide();$(".calendar .cal-cell").popover("hide");$("#add-section").fadeIn("fast");if(0==$("div.wysiwyg").length){var b=new Wysiwyg;$(".area").before(b.el)}$(this).hide();ready_time($("input#time"))});$("#hide-add").live("click",function(b){b.preventDefault();$("div.wysiwyg").remove();$("#add-section").hide();$("#show-add").fadeIn("fast");
$("#event-container").fadeIn("fast")});$("#main-add").live("click",function(b){b.preventDefault();b=$("h2#active-date").html().slice(0,-1);var d=$("#time").val(),c=$("#event").val(),e=$("div.area")[0].innerHTML;""!=b&""!=c?$.ajax({type:"POST",url:config.add_event,dataType:"json",data:{day:b,time:d,event:c,description:e},success:function(b){"Added"==b.result?(change_count("+"),add_event(b.id,d,c,e),$("p.event-info").remove(),$("div#event-container > div").tsort("span.time"),$("#hide-add").trigger("click"),
$("div#event-container").response(ui_lang.cal_added,!0)):$(".popover-add").response(b.result,!1)},fail:function(){$(".popover-add").response(ui_lang.server_side_error,!1)}}):$(".popover-add").response(ui_lang.no_values,!1)});$("form :input").change(function(){$(this).closest("form").data("changed",!0)});$("ul.nav-tabs li a").click(function(){$.totalStorage("opened_tab",$(this).attr("href"))});$("#userSettings").click(function(b){b.preventDefault();b=$("#user_settings");if($(this).closest("form").data("changed")){var d=
!1;$.each(b.find("input"),function(){if(""==this.value||void 0==this.value)d=!0});d?$("#user_settings").response(ui_lang.no_values,!1):(b=$("#user_settings"),b.attr("action",config.home+"settings/process_user"),b.submit())}else $("#user_settings").response(ui_lang.no_change,!1)});$("a.bt_hide").live("click",function(b){b.preventDefault();b=$(this).attr("href");$(b).toggle();$(b).next().toggle();$(this).children("i").toggleClass("icon-chevron-up");$(this).children("i").toggleClass("icon-chevron-down");
set_hidden(b)});$("#delete_acc_button").click(function(){if(confirm(ui_lang.delete_acc))window.location="http://ddiary.loc/logout/app/delete";else return!1});$("#access_log").click(function(){$.get(config.access,function(b){b=eval(b);b[0].message||(2>$("#accessTable tr").length&&b.forEach(function(b){$("#accessTable").append("<tr><td>"+b.ip+"</td><td>"+b.time+"</td><td>"+b.client+"</td></tr>")}),$("#accessModal").modal("show"))})})});

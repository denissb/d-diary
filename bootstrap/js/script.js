...
function hide_cal() {
	if(typeof(Storage) !== "undefined") {
		...
		if(localStorage.hidden !== "undefined") {
			hidden = JSON.parse(localStorage.hidden);
			if(hidden.indexOf("tbody#cal-body") != -1) {
				$("tbody#cal-body").hide();
				var button = $("button.toggle-cal");
				button.children('i').toggleClass('icon-chevron-down');
				button.children('i').toggleClass('icon-chevron-up');
				button.show();
			}
		}	
	}
